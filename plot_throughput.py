...
# Global styling tweaks for a cleaner look
mpl.rcParams.update({
...
# Fixed palette for all plots
PALETTE_NAME = 'Set2'

def _discover_models(results_dir: Path):
    """Discover model subdirectories present in result groups."""
    configurations = {}
        ...
        if result_group.is_dir() and result_group.name.endswith('_results'):
            ...
            if serve_dir.exists():
                for model_dir in serve_dir.iterdir():
                    if model_dir.is_dir():
                        configurations[model_dir.name] = model_dir.name
...
def get_display_name(model, display_names):
    """Get display name for a model, or return model name if not in config"""
...
def create_all_scenario_plots(all_data):
    """Create plots for each scenario and save to PDF."""
    try:
    ...
    for scenario, data in all_data.items():
        ...
        # Use custom colors from config if available, otherwise use palette
        page_model_colors = {}
        ...
        # Generate colors for models without custom colors
        ...
        plt.close(fig)
        print(f"✓ Created plot for: {scenario_titles.get(scenario, scenario)}")
...
if __name__ == "__main__":
    ...
    all_data = collect_all_scenario_data()

    print("\nCreating throughput comparison charts for each scenario...")
